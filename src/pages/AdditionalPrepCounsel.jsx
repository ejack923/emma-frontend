...
const SectionHeading = ({ children }) => (
  <h2 className="text-base font-bold text-purple-800 mt-8 mb-3 border-b border-purple-100 pb-1">{children}</h2>
);
...
const TextInput = ({ value, onChange, placeholder = "", type = "text" }) => (
  <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
    className="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300" />
);
const NumInput = ({ value, onChange }) => (
  <input type="number" min="0" value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)}
    className="border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 w-24" />
);
const TextArea = ({ value, onChange, rows = 3, placeholder = "" }) => (
  <textarea value={value} onChange={e => onChange(e.target.value)} rows={rows} placeholder={placeholder}
    className="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 resize-y" />
);
...
export default function AdditionalPrepCounsel() {
  ...
  const handleSubmit = async (e) => {
    ...
    await base44.integrations.Core.SendEmail({
      ...
      subject: `Additional Prep Worksheet (Counsel) – ${form.clientName || "Unknown Client"} (${form.vlaRefNo || "No Ref"})`,
      body: `
...
PART C: RELEVANT GUIDELINE (fee schedule 4)
...
Grant of assistance warranted: ${form.satisfiedGrant}
  ...
  Less 8 hours: ${billableHours.toFixed(2)} hours at $${hourlyRate}/hr (${form.court})
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-xl shadow p-10 max-w-md w-full text-center">
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => setSubmitted(false)} className="bg-purple-700 text-white px-6 py-2 rounded-lg text-sm hover:bg-purple-800 transition-colors">Submit Another</button>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="py-10 px-4">
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
          <div className="bg-gradient-to-r from-purple-700 to-purple-800 px-8 py-6 flex items-start justify-between">
            <div>
              <p className="text-purple-200 text-xs mb-1">Victoria Legal Aid — Application for Extension of Legal Assistance</p>
              <h1 className="text-white text-xl font-bold">Preparation Fees Worksheet/Checklist</h1>
              <h1 className="text-white text-xl font-bold">Counsel — 2026 Fees</h1>
              <p className="text-purple-200 text-xs mt-2">For use by Section 29A Panel Practitioners (Simplified Grants Process)</p>
            </div>
            <div className="bg-white rounded-lg px-4 py-2 text-right flex-shrink-0">
              <p className="text-purple-700 font-bold text-sm leading-tight">Victoria</p>
              <p className="text-purple-700 font-bold text-sm leading-tight">Legal Aid</p>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-6">
            ...
            <div className="p-3 mb-4 bg-purple-50 border border-purple-200 rounded text-sm text-purple-800">
              <strong>Note:</strong> Where two Counsel are engaged, Junior Counsel is only entitled to perusals.
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
              ...
              <div className="flex gap-6 mt-2">
                {["satisfied", "not satisfied"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="satisfiedPrep" value={v} checked={form.satisfiedPrep === v} onChange={() => set("satisfiedPrep", v)} className="w-4 h-4 accent-purple-700" />
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-6 mt-2">
                {["satisfied", "not satisfied"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="satisfiedGrant" value={v} checked={form.satisfiedGrant === v} onChange={() => set("satisfiedGrant", v)} className="w-4 h-4 accent-purple-700" />
                    <span className="text-sm font-semibold capitalize">{v}</span>
                  </label>
                ))}
              </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
              <p className="text-sm font-bold text-purple-800 mb-3">Calculation Summary</p>
              <div className="space-y-1 text-sm text-slate-700">
                ...
                <div className="flex justify-between font-semibold border-t border-purple-200 pt-1 mt-1"><span>Subtotal:</span><span>{subtotal.toFixed(2)} hours</span></div>
                <div className="flex justify-between text-slate-500"><span>Less 8 hours at ${hourlyRate}/hr {form.court ? `(${form.court})` : ""}:</span><span>{billableHours.toFixed(2)} hrs</span></div>
                <div className="flex justify-between font-bold text-purple-800 border-t border-purple-300 pt-1 mt-1 text-base"><span>Preparation Amount:</span><span>${preparationAmount.toFixed(2)}</span></div>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-6 mt-2 mb-2">
                {["Yes", "No"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="previousGrantMade" value={v} checked={form.previousGrantMade === v} onChange={() => set("previousGrantMade", v)} className="w-4 h-4 accent-purple-700" />
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={form.meansConfirm} onChange={e => set("meansConfirm", e.target.checked)} className="w-4 h-4 mt-0.5 accent-purple-700" />
            ...
            <label className="flex items-start gap-3 cursor-pointer mb-4">
              <input type="checkbox" checked={form.practitionerConfirm} onChange={e => set("practitionerConfirm", e.target.checked)} className="w-4 h-4 mt-0.5 accent-purple-700" />
            ...
            <div className="mt-8 flex justify-between items-center gap-3 no-print">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const content = `COUNSEL PREPARATION FEES\n\nClient: ${form.clientName}\nVLA Ref: ${form.vlaRefNo}\nCharges: ${form.chargeType}\nCourt: ${form.court}\n\nAmount: $${preparationAmount.toFixed(2)}`;
                    addToBundle("Additional Prep (Counsel)", content);
                  }}
                  className="bg-purple-600 hover:bg-purple-700 text-white font-medium px-6 py-3 rounded-lg transition-colors text-sm"
                >
                  Add to Bundle
                </button>
                <button type="submit" disabled={submitting}
                  className="bg-purple-700 hover:bg-purple-800 text-white font-semibold px-8 py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
