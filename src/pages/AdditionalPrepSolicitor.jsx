...
const SectionHeading = ({ children }) => (
  <h2 className="text-base font-bold text-green-800 mt-8 mb-3 border-b border-green-100 pb-1">{children}</h2>
);
...
const TextInput = ({ value, onChange, placeholder = "", type = "text" }) => (
  <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
    className="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-300" />
);
const NumInput = ({ value, onChange }) => (
  <input type="number" min="0" value={value} onChange={e => onChange(parseFloat(e.target.value) || 0)}
    className="border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-300 w-24" />
);
const TextArea = ({ value, onChange, rows = 3, placeholder = "" }) => (
  <textarea value={value} onChange={e => onChange(e.target.value)} rows={rows} placeholder={placeholder}
    className="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-300 resize-y" />
);
...
export default function AdditionalPrepSolicitor() {
  const [form, setForm] = useState({
    clientName: "", vlaRefNo: "",
    chargeType: "", chargeDetails: "", caseType: "", stage: "", court: "",
  ...
  const billableHours = Math.min(15, Math.max(0, perusalsHours - 20));
  const hourlyRate = COURT_RATES[form.court] || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    await base44.integrations.Core.SendEmail({
      ...
      subject: `Additional Prep Worksheet (Solicitor) – ${form.clientName || "Unknown Client"} (${form.vlaRefNo || "No Ref"})`,
      body: `
PREPARATION FEES WORKSHEET/CHECKLIST FOR SOLICITOR - 2026 FEES
...
PART C: RELEVANT GUIDELINE (fee schedule 1)
...
Grant of assistance warranted: ${form.satisfiedGrant}
  ...
  Total (max 15 hours): ${billableHours.toFixed(2)} hours at $${hourlyRate}/hr (${form.court})
  PREPARATION AMOUNT: $${preparationAmount.toFixed(2)}

Previous grant of aid made: ${form.previousGrantMade}
Previous grant details: ${form.previousGrantDetails}

PART E: MEANS
Client financial details unchanged: ${form.meansConfirm ? "Confirmed" : "Not confirmed"}

PART F: PRACTITIONER CERTIFICATION
Confirmed: ${form.practitionerConfirm ? "Yes" : "No"}
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-xl shadow p-10 max-w-md w-full text-center">
          ...
          <p className="text-slate-500 text-sm mb-6">Sent to ejackson@completelawsupport.com</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => setSubmitted(false)} className="bg-green-700 text-white px-6 py-2 rounded-lg text-sm hover:bg-green-800 transition-colors">Submit Another</button>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="py-10 px-4">
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
          <div className="bg-gradient-to-r from-green-700 to-green-800 px-8 py-6 flex items-start justify-between">
            <div>
              <p className="text-green-200 text-xs mb-1">Victoria Legal Aid — Application for Extension of Legal Assistance</p>
              <h1 className="text-white text-xl font-bold">Preparation Fees Worksheet/Checklist</h1>
              <h1 className="text-white text-xl font-bold">Solicitor — 2026 Fees</h1>
              <p className="text-green-200 text-xs mt-2">For use by Section 29A Panel Practitioners (Simplified Grants Process)</p>
            </div>
            <div className="bg-white rounded-lg px-4 py-2 text-right flex-shrink-0">
              <p className="text-green-700 font-bold text-sm leading-tight">Victoria</p>
              <p className="text-green-700 font-bold text-sm leading-tight">Legal Aid</p>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div><FieldLabel>Details (if required)</FieldLabel><TextInput value={form.chargeDetails} onChange={v => set("chargeDetails", v)} placeholder="Additional charge details..." /></div>
              <div>
              ...
              <div>
                ...
                <select value={form.court} onChange={e => set("court", e.target.value)}
                  ...
                  {Object.keys(COURT_RATES).map(c => <option key={c} value={c}>{c} (${COURT_RATES[c]}/hr)</option>)}
                </select>
              </div>
            </div>
            ...
            <p className="text-sm text-slate-600 mb-3">The relevant guideline is contained in fee schedule 1 of the Legal Aid Handbook (Part 23).</p>
            <div className="mb-4">
              ...
              <div className="flex gap-6 mt-2">
                {["satisfied", "not satisfied"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="satisfiedPrep" value={v} checked={form.satisfiedPrep === v} onChange={() => set("satisfiedPrep", v)} className="w-4 h-4 accent-green-700" />
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-6 mt-2">
                {["satisfied", "not satisfied"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="satisfiedGrant" value={v} checked={form.satisfiedGrant === v} onChange={() => set("satisfiedGrant", v)} className="w-4 h-4 accent-green-700" />
                    <span className="text-sm font-semibold capitalize">{v}</span>
                  </label>
                ))}
              </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
              <p className="text-sm font-bold text-green-800 mb-3">Calculation Summary</p>
              <div className="space-y-1 text-sm text-slate-700">
                ...
                <div className="flex justify-between font-semibold border-t border-green-200 pt-1 mt-1"><span>Total (max 15 hours):</span><span>{billableHours.toFixed(2)} hrs at ${hourlyRate}/hr {form.court ? `(${form.court})` : ""}</span></div>
                <div className="flex justify-between font-bold text-green-800 border-t border-green-300 pt-1 mt-1 text-base"><span>Preparation Amount:</span><span>${preparationAmount.toFixed(2)}</span></div>
              </div>
            </div>

            <div className="mb-4">
              <FieldLabel>Has a previous grant of aid been made for solicitor additional preparation in this matter?</FieldLabel>
              <div className="flex gap-6 mt-2 mb-2">
                {["Yes", "No"].map(v => (
                  <label key={v} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="previousGrantMade" value={v} checked={form.previousGrantMade === v} onChange={() => set("previousGrantMade", v)} className="w-4 h-4 accent-green-700" />
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={form.meansConfirm} onChange={e => set("meansConfirm", e.target.checked)} className="w-4 h-4 mt-0.5 accent-green-700" />
            ...
            <SectionHeading>Part F: Practitioner Certification</SectionHeading>
            <label className="flex items-start gap-3 cursor-pointer mb-4">
              <input type="checkbox" checked={form.practitionerConfirm} onChange={e => set("practitionerConfirm", e.target.checked)} className="w-4 h-4 mt-0.5 accent-green-700" />
              <span className="text-sm text-slate-700">I have attached a letter / memorandum setting out why the grant is justified. I request that assistance be granted at the amount calculated.</span>
            </label>
            ...
            <div className="mt-8 flex justify-between items-center gap-3 no-print">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const content = `SOLICITOR PREPARATION FEES\n\nClient: ${form.clientName}\nVLA Ref: ${form.vlaRefNo}\nCharges: ${form.chargeType}\nCourt: ${form.court}\n\nAmount: $${preparationAmount.toFixed(2)}`;
                    addToBundle("Additional Prep (Solicitor)", content);
                  }}
                  className="bg-green-600 hover:bg-green-700 text-white font-medium px-6 py-3 rounded-lg transition-colors text-sm"
                >
                  Add to Bundle
                </button>
                <button type="submit" disabled={submitting}
                  className="bg-green-700 hover:bg-green-800 text-white font-semibold px-8 py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
